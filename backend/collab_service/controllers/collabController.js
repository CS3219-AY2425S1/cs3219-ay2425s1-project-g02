...
class CollabController {
  handleSocketEvents = (io, socket) => {
    ...
    socket.on("languageUpdate", (data) => {
      ...
      console.log(
        `Received language update request for session: ${sessionIdObj}`
      );
  ...
  handleSessionCreated = async (req, res) => {
    ...
    if (!sessionId || !sessionData || !questionData) {
      console.error("Missing sessionId or sessionData or questionData:", {
        sessionId,
        sessionData,
        questionData,
      });
      return res
        ...
        .json({
          message:
            "All of sessionId, sessionData, and questionData are required.",
        });
    }
